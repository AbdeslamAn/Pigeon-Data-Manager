...
if (isset($_POST['n_bage']) &&
    ...
    isset($_POST['id_pig_sq'])) {
		...
		if (mysqli_num_rows($result) <= 0) {
			header("Location: pigeons.php?ms=Pigeon not found");
			exit;
		}
	...
	$user_email = $_POST['user_email'];
		...
		if (isset($_FILES['picture']['name']) && !empty($_FILES['picture']['name'])) {
			...
			if (in_array($img_ex_lc, $allowed_exs)) {
				...
				$img_upload_path = '../user/php/uploads/'.$new_img_name;
		...
		if (isset($_FILES['picture2']['name']) && !empty($_FILES['picture2']['name'])) {
			...
			if (in_array($img_ex_lc2, $allowed_exs2)) {
				...
				$img_upload_path2 = '../user/php/uploads/'.$new_img_name2;
        ...
        if (isset($_FILES['picture3']['name']) && !empty($_FILES['picture2']['name'])) {
			...
			if (in_array($img_ex_lc3, $allowed_exs3)) {
				...
				$img_upload_path3 = '../user/php/uploads/'.$new_img_name3;
				move_uploaded_file($tmp_name3, $img_upload_path3);

				// Update the mother's pedigree image path
				$sql = "UPDATE pigeon_squab SET ped_mother='$img_upload_path3' WHERE id_pig_sq='$id_pig_sq'";
				mysqli_query($connection, $sql);
			}
		}

		// Update other pigeon information
		$sql = "UPDATE pigeon_squab SET n_bage='$n_bage', strains='$straines',pig_squab='$pig_squab', name_pig='$name_pigeon',color='$color',gander='$gander', n_father='$bage_father', n_mother='$bage_mother', not_info='$note',user_email='$user_email' WHERE id_pig_sq='$id_pig_sq' ";
		$result = mysqli_query($connection, $sql);

		if ($result) {
			$ms = "<h3 style='background-color:lightgray;border-radius:20px;width:300px;'>Successfully Updated.</h3>";
			header("Location: edit_pigeon.php?ms=$ms&id_pig_sq=$id_pig_sq");
			exit; 
		} else {
		...
		header("Location: pigeons.php?ms=hello");
		exit;
	}
?>
