...
if (isset($_GET['id'])) {
    ...
    if ($result) {
        ...
        $message = "<h3 style=background-color:lightgray;border-radius:20px;width:300px;>User deleted successfully.</h3>";
    } else {
        // Deletion failed
        $message = "<h3 style=background-color:lightgray;border-radius:20px;width:300px;>Failed to delete user.</h3>";
    }

    // Redirect back to the main page with a message
    header("Location: ../users.php?ms=" . urlencode($message));
    exit();
} else {
